...
<body>
	...
	<ol>
		...
		<li>발급받은 세션ID는 클라이언트측의 웹 브라우저 특정 부분(쿠키)에 저장된다
			(같은 IP의 다른 웹 브라우저로 접속하면 새로운 세션ID를 발급받는다)</li>
		<li>이후 해당 클라이언트는 해당 서버로 요청을 보낼때마다 세션ID를 요청에 함께 실어 전달하게 된다</li>
		<li>서버는 실려온 세션 ID를 클라이언트를 구분할 수 있다.</li>
		<li>세션ID는 개인 사물함 열쇠 같은 역할을 하기도 한다.
			세션 어트리뷰트에 접근할 때 해당 세션 ID를 열쇠로 사용한다.</li>
		<li>세션 객체에 너무 많은 양의 데이터를 추가하는 것은 서버의 성능을 저하시킬 수 있으므로 주의해야 한다</li>
		<li>세션ID는 기본적으로 사용자의 웹 브라우저가 종료될 때 함께 사라지도록 설정되어 있다</li>
		<li>세션 어트리뷰트는 일정시간동안 요청을 받지 않으면 사라지도록 설정되어 있다.</li>
	</ol>
	<%-- 
		sessionAttribute = {
			D1B223BA48E5B95234E4AC1C881F937B: {
				'food': 'orange',
				'age': 20
			},
			EAD354BD48E5B95234E4AC1C881F937B: {
				'food': 'oragne',
			}
		}
	--%>
